...
{                                                 			\
	...
	for (i = 0; node->nbrs[i]; ++i) {				\
		next_hop = node->nbrs[i];				\
		for(j = 0;next_hop->nbrs[j]; ++j) {			\
			if (strcmp(node->node_name, next_hop->nbrs[j]->node_name) == 0) continue;	\
			nbrnode = next_hop->nbrs[j];		\
